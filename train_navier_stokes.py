...
from config.config import Navier_Stokes_Config as Config
...
def run_train(cfg: Config):
    ...
    for epoch in trange(cfg.num_epochs, desc="Training"):
        ...
        # u_pred =  torch.tensor(y_list[0], dtype=torch.float32, device=device).unsqueeze(0).repeat(cfg.batch_size, )
        # t_prev = t_prev.item()
