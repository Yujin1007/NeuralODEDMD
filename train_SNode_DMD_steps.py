...
from dataset.generate_synth_dataset import load_synth, SynthDataset_seq
...
def run_train(cfg: Stochastic_Node_DMD_Config, K=3):
    set_seed(cfg.seed)
    device = torch.device(cfg.device)
    t_list, coords_list, y_list, *_ = load_synth(device, T=cfg.data_len)
    dataset = SynthDataset_seq(t_list, coords_list, y_list, K=K)
    ...
    pretrained_path = "results/stochastic/run28"
    ...
    for epoch in trange(cfg.num_epochs, desc="Training"):
        ...
        for batch in dataloader:
            t_next_k, coords_k, y_next_k, y_prev_k = batch
            t_next_k = [x.to(device) for x in t_next_k]
            coords_k = [x.to(device) for x in coords_k]
            y_next_k = [x.to(device) for x in y_next_k]
            y_prev_k = [x.to(device) for x in y_prev_k]
            ...
            # print(f"tiem : t_next {y_prev_k}, \nshape {y_prev_k[1].shape} ")
            # t_next = t_next_k[0]          # converts 0-dim tensor or [1] tensor to a Python float
            # coords   = coords.squeeze(0)     # [1,102,2] -> [102,2]
            # y_next   = y_next.squeeze(0)       # [1,102,2] -> [102,2]
            # y_prev   = y_prev.squeeze(0)       # [1,102,2] -> [102,2]

            opt.zero_grad()
            y_prev   = y_prev_k[0].squeeze(0)       # [1,102,2] -> [102,2]
            u_pred = y_prev
            K_loss = 0.
            for k in range(K):
                t_next = t_next_k[k].item()          # converts 0-dim tensor or [1] tensor to a Python float
                coords   = coords_k[k].squeeze(0)     # [1,102,2] -> [102,2]
                y_next   = y_next_k[k].squeeze(0)       # [1,102,2] -> [102,2]
                ...
                t_prev = t_next
                k_loss, parts = stochastic_loss_fn(
